...
export const QUESTIONNAIRES: Readonly<Record<ProductSlug, Questionnaire>> = {
  ...
  'career-money': {
    steps: [
      ...
      { id: 'status', title: 'How are you employed?', fields: [
  ...
  relationship: {
    steps: [
      identity,
      { id: 'status', title: 'Where do things stand?', fields: [
        { key: 'status', kind: 'radio', label: 'Relationship status', required: true, options: ['Dating', 'Engaged', 'Married', 'Separated', 'It is complicated', 'Single and deciding'] },
        { key: 'duration', kind: 'radio', label: 'How long', required: true, options: ['Under a year', '1 to 3 years', '3 to 7 years', '7 to 15 years', 'Over 15 years'] }] },
      { id: 'partner', title: 'Tell us about them',
        intro: 'Optional — but this is what turns the report from a reading of you into a reading of the two of you. Without it we can only work from your side.',
        fields: [
          { key: 'partnerName', kind: 'text', label: 'Their full name', maxLength: 80 },
          { key: 'partnerDob', kind: 'date', label: 'Their date of birth' }] },
      { id: 'concern', title: 'What brought you here?', fields: [
        { key: 'concern', kind: 'radio', label: 'Main concern', required: true, options: ['We keep having the same fight', 'We have grown apart', 'Trust has been broken', 'Family is against it', 'They will not commit', 'I am not sure I want this', 'Something else'] }] },
      { id: 'pattern', title: 'How do the two of you talk?', fields: [
        { key: 'communication', kind: 'radio', label: 'Communication', required: true, options: ['Openly, most of the time', 'Only when things are calm', 'We avoid the hard things', 'It turns into an argument', 'We barely talk now'] },
      ...
      { id: 'outcome', title: 'What do you want to happen?', fields: [
        ...
        { key: 'context', kind: 'textarea', label: 'Anything else we should know', maxLength: 600 }] },
      contact,
    ],
    refinements: [
